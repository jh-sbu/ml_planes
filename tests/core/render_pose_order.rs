...
//!
//! Network interpolation writes each rendered `Transform` during `Update`, while
//! the camera and gizmo systems read it in the same schedule. The feature-gated
//! systems cannot run in this headless test, so it pins their ordering contract:
//! everything in
...
/// `reader_saw` is what a reader observed.
...
/// The ordering must hold across frames.
#[test]
