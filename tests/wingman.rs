...
use ml_planes::controllers::{
    ActiveController, LevelHoldController, WingmanController, FormationOffset, LeaderRef, LeaderState,
};
...
fn wingman_and_leader_stay_airborne() {
    ...
    app.world_mut().entity_mut(wingman).insert((
        LeaderRef(leader),
        LeaderState::default(),
        offset,
    ));
    ...
    for state in &states {
        assert!(state.altitude.is_finite(), "altitude not finite: {}", state.altitude);
        assert!(state.airspeed.is_finite(), "airspeed not finite: {}", state.airspeed);
        assert!(
            state.altitude > 0.0,
...
fn wingman_follows_leader_altitude_change() {
    ...
    {
        let mut ctrl_q = app.world_mut()
            .query::<&mut ActiveController>();
    ...
    let states = read_states(&mut app);
    let wingman_state = app.world().entity(wingman).get::<FlightState>().unwrap().clone();
    ...
    for state in &states {
        assert!(state.altitude > 0.0, "plane crashed: altitude={}", state.altitude);
    }
}
